...
import { Trophy, RotateCcw, User, Download } from 'lucide-react';
...
const ResultPage: React.FC<ResultPageProps> = ({ playerName, score, gameId, onRestart }) => {
  ...
  const downloadExcel = () => {
    ...
    // Download file
    XLSX.writeFile(workbook, `DinoJumper_Leaderboard_${new Date().toISOString().split('T')[0]}.xlsx`);
  };
      ...
      <div className="w-full max-w-5xl grid grid-cols-1 md:grid-cols-2 gap-8 items-stretch">
        ...
        >
          ...
          <div className="space-y-4 mt-8">
            <motion.div 
              ...
              className="p-4 bg-green-500/10 border-2 border-green-500/20 rounded-xl flex flex-col items-center justify-center gap-1 text-green-600"
            >
               ...
               <span className="text-[8px] opacity-60 uppercase font-press-start">Press [1] to Save Record</span>
            </motion.div>
            ...
            >
              ...
              <span className="text-[8px] opacity-60">PRESS [ENTER] TO SAVE & RESTART</span>
            </motion.button>
          </div>
        </motion.div>
        ...
        >
          <div className="flex items-center justify-between mb-8">
            <h3 className="text-lg uppercase tracking-tight flex items-center gap-3">
              <Trophy size={20} className="text-yellow-500" />
              Top Jumper
            </h3>
            <span className="text-[10px] text-game-text/40">GLOBAL (LOCAL)</span>
          </div>
          ...
          <div className="mt-8 pt-6 border-t-2 border-dashed border-game-text/10 text-center space-y-4">
            <div className="flex flex-col gap-3">
              <button 
                ...
                className="flex items-center justify-center gap-2 w-full p-3 bg-game-text text-white rounded-lg text-[10px] uppercase font-bold hover:bg-game-text/90 transition-colors"
              >
                <Download size={14} />
            ...
            <p className="text-[8px] text-game-text/40 leading-relaxed">
              *LEADERBOARD IS STORED LOCALLY ON YOUR BROWSER.<br />
              HIGH SCORES ARE PERSISTENT ACROSS SESSIONS.
            </p>
          </div>
        </motion.div>

...
type GameState = 'start' | 'rules' | 'playing' | 'result' | 'admin';
...
const App: React.FC = () => {
  ...
  const [appSettings, setAppSettings] = useState<AppSettings>(() => {
    const saved = localStorage.getItem('dino_app_settings');
  ...
  const updateAppSettings = (newSettings: AppSettings) => {
    ...
    localStorage.setItem('dino_app_settings', JSON.stringify(newSettings));
  };
  ...
  return (
    <div 
      className="min-h-screen flex flex-col items-center justify-center p-4 text-game-text font-press-start selection:bg-game-accent selection:text-white transition-all duration-700"
      style={{ 
        backgroundColor: appSettings.backgroundColor,
        backgroundImage: appSettings.backgroundImageUrl ? `url(${appSettings.backgroundImageUrl})` : 'none',
    ...
    >
      ...
      <div className="fixed top-6 left-6 z-50 pointer-events-none sm:pointer-events-auto">
        <motion.img 
          initial={false}
          ...
          alt="Dino Jump Logo" 
          className="w-12 h-12 md:w-20 md:h-20 rounded-lg border-2 border-game-text bg-white shadow-[4px_4px_0_0_#333] transition-transform hover:scale-110 object-contain p-1"
          onError={(e) => {
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <button 
                    ...
                    className="p-2 hover:bg-game-bg rounded-lg transition-colors text-game-text/50 hover:text-game-accent"
                    title="Admin Panel"
                  >
                    <Settings size={14} />
                  </button>
